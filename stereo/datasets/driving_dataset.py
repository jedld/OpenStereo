...
class DrivingDataset(DatasetTemplate):
    def __init__(self, data_info, data_cfg, mode):
        ...
        self.use_segmentation = self.data_info.get('USE_SEGMENTATION', False)
    ...
    def __getitem__(self, idx):
        ...
        if self.use_segmentation:
            left_img_path, right_img_path, disp_img_path, seg_img_path = full_paths
        else:
        ...
        if self.use_segmentation:
            seg_img = np.array(Image.open(seg_img_path).convert('RGB'), dtype=np.float32)
